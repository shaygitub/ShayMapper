...
uint64_t kdmapper::AllocMdlMemory(uint64_t size, uint64_t* mdlPtr) {
	...
	auto mdl = kernel_api::MmAllocatePagesForMdl(LowAddress, HighAddress, LowAddress, pages * (uint64_t)PAGE_SIZE);
	if (!mdl) {
	...
	if (!DriverMapper::ReadMemory(mdl + 0x028 /*_MDL : byteCount*/, &byteCount, sizeof(uint32_t))) {
	...
	if (byteCount < size) {
		...
		kernel_api::MmFreePagesFromMdl(mdl);
		kernel_api::FreePool(mdl);
		return { 0 };
	}

	auto mappingStartAddress = kernel_api::MmMapLockedPagesSpecifyCache( mdl, nt::KernelMode, nt::MmCached, NULL, FALSE, nt::NormalPagePriority);
	...
	const auto result = kernel_api::MmProtectMdlSystemAddress(mdl, PAGE_EXECUTE_READWRITE);
	if (!result) {
		Log(L"[-] Can't change protection for mdl pages, cleaning up" << std::endl);
		kernel_api::MmUnmapLockedPages(mappingStartAddress, mdl);
		kernel_api::MmFreePagesFromMdl(mdl);
		kernel_api::FreePool(mdl);
		return { 0 };
	}
...
uint64_t kdmapper::AllocIndependentPages(uint32_t size)
{
	const auto base = kernel_api::MmAllocateIndependentPagesEx(size);
	if (!base)
	{
	...
	if (!kernel_api::MmSetPageProtection(base, size, PAGE_EXECUTE_READWRITE))
	{
		Log(L"[-] Failed to change page protections" << std::endl);
		kernel_api::MmFreeIndependentPages(base, size);
		return 0;
	}
...
uint64_t kdmapper::MapDriver(BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	if (mode == AllocationMode::AllocateMdl) {
		...
		kernel_image_base = AllocIndependentPages(image_size);
	}
	else { // AllocatePool by default
		kernel_image_base = kernel_api::AllocatePool(nt::POOL_TYPE::NonPagedPool, image_size);
	}
	...
	do {
		...
		if (!DriverMapper::WriteMemory(realBase, (PVOID)((uintptr_t)local_image_base + (destroyHeader ? TotalVirtualHeaderSize : 0)), image_size)) {
		...
		NTSTATUS status = 0;
		if (!DriverMapper::CallKernelFunction(&status, address_of_entry_point, (PassAllocationAddressAsFirstParam ? realBase : param1), param2)) {
		...
		if (free) {
			...
			if (mode == AllocationMode::AllocateMdl) {
				free_status = kernel_api::MmUnmapLockedPages(realBase, mdlptr);
				free_status = (!free_status ? false : kernel_api::MmFreePagesFromMdl(mdlptr));
				free_status = (!free_status ? false : kernel_api::FreePool(mdlptr));
			}
			else if (mode == AllocationMode::AllocateIndependentPages)
			{
				free_status = kernel_api::MmFreeIndependentPages(realBase, image_size);
			}
			else {
				free_status = kernel_api::FreePool(realBase);
			}
	...
	if (mode == AllocationMode::AllocateMdl) {
		free_status = kernel_api::MmUnmapLockedPages(kernel_image_base, mdlptr);
		free_status = (!free_status ? false : kernel_api::MmFreePagesFromMdl(mdlptr));
		free_status = (!free_status ? false : kernel_api::FreePool(mdlptr));
	}
	else if (mode == AllocationMode::AllocateIndependentPages)
	{
		free_status = kernel_api::MmFreeIndependentPages(kernel_image_base, image_size);
	}
	else {
		free_status = kernel_api::FreePool(kernel_image_base);
	}
...
bool kdmapper::ResolveImports(portable_executable::vec_imports imports) {
	for (const auto& current_import : imports) {
		...
		for (auto& current_function_data : current_import.function_datas) {
			uint64_t function_address = DriverMapper::GetKernelModuleExport(Module, current_function_data.name);

			if (!function_address) {
				//Lets try with ntoskrnl
				if (Module != *((ULONG64*)DriverMapper::GetLoaderResource(L"ntoskrnlAddr"))) {
					function_address = 
						DriverMapper::GetKernelModuleExport(*((ULONG64*)DriverMapper::GetLoaderResource(L"ntoskrnlAddr")), current_function_data.name);
					if (!function_address) {
#if !defined(DISABLE_OUTPUT)
